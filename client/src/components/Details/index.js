  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/devices/${id}`).then((response) => {
      ...
      .then((res) => {
        ...
        data.reverse()
        data = data.map((item, key) => ({...item, color: colors[key % colors.length], key:key}))
        console.log(data)
        setData(data)
        setMaxi(maxi)
          ...
          <div className='stats d-inline-flex'>
            <div className='pie_chart '>
              ...
              <VictoryPie
                ...
                style={{
                  data: {
                    fillOpacity: 0.9, stroke: "white", strokeWidth: 3, width: '20vw'
                  }
                ...
                radius={({ datum }) => {
                  // let radi = (datum.y / maxi * 250)
                  let radi = 270 - datum.key* 20
                  // console.log(radi,maxi)
                  return radi
                }}
